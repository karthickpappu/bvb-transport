...
Class UsersModel extends CI_Model {
    ...
    {					      
        ...
        { 
			...
			$data['department_id']	    = $post_data['department_id'];
			$data['designation_id']	 	= $post_data['designation_id'];
			$data['reporting_to']	    = $post_data['reporting_to'];
			// $data['reporting_to']	    = implode(',',$post_data['reporting_to']);
			$data['description']		= $post_data['description'];
			$data['address']	    	= $post_data['address'];
			$data['state']		        = $post_data['state'];
            ...
            if(strlen($_FILES['user_pic']['name']) > 0){
                ...
                $data['user_pic'] 				    = $post_data['user_pic_old'];
            }
            ...
            { 
                    $status = '1';      
                    $sql = "SELECT * FROM users WHERE user_id = ?  AND status = ? LIMIT 1";
                    $query = $this->db->query($sql, [$user_id,$status]);
                    $result = $query->row();  
                    if($result){
                        return $result;
                    }else{
    ...
    function updatepassword($post_data,$token)
    {					      
		try 
        { 

            $user_password = $post_data['user_password'];       
            $user_password = $post_data['opasscode'];      
			$hashed_password = $this->crypt->hash_password($user_password);
            $sql = "SELECT * FROM users WHERE  passcode = ? ";
            $query = $this->db->query($sql, [$hashed_password]);
            $result = $query->row();  
            if($result){

                $id                 = $post_data['user_id'];   
                $password           = $post_data['passcode'];                
			    $data['passcode']   = $this->crypt->hash_password($password);
